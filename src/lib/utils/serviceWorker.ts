...
class ServiceWorkerManager {
  ...
  async register(): Promise<boolean> {
    if (typeof window === 'undefined' || !('serviceWorker' in navigator)) {
      // logWarn('[SW] Service Workers not supported')
      return false
    }
  ...
  private notifyUpdateCallbacks(update: ServiceWorkerUpdate) {
    this.updateCallbacks.forEach(callback => {
      try {
        ...
        // logError('[SW] Update callback error:', error)
      }
    })
  }
  ...
  async skipWaiting(): Promise<void> {
    if (!this.registration?.waiting) {
      // logWarn('[SW] No waiting service worker to skip')
      return
    }
  ...
  async update(): Promise<boolean> {
    if (!this.registration) {
      // logWarn('[SW] No registration available for update')
      return false
    }
    ...
    try {
      ...
      // logError('[SW] Update check failed:', error)
      return false
    }
  }
  ...
  async unregister(): Promise<boolean> {
    ...
    try {
      ...
      // logError('[SW] Failed to unregister service worker:', error)
      return false
    }
  }
  ...
  async clearAllCaches(): Promise<void> {
    try {
      ...
      // logError('[SW] Failed to clear caches:', error)
    }
  }
  ...
  async sendMessage(type: string, payload?: any): Promise<any> {
    if (!this.registration?.active) {
      // logWarn('[SW] No active service worker to send message to')
      return null
    }
  ...
  async getCacheInfo() {
    try {
      ...
      // logError('[SW] Failed to get cache info:', error)
      return null
    }
  }
  ...
  async cacheResource(url: string): Promise<boolean> {
    try {
      ...
      // logError('[SW] Failed to cache resource:', error)
      return false
    }
  }
  ...
  async getVersion(): Promise<string | null> {
    try {
      ...
      // logError('[SW] Failed to get version:', error)
      return null
    }
  }
