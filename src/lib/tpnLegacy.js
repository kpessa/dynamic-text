...
class TPNLegacySupport {
  constructor() {
    ...
    this.values = {}; // Store current values
    this.data = {
      RREC: {
        INGREDIENT_map: {},
        META: {
   ...
   */
  setValues(valueMap) {
    Object.assign(this.values, valueMap);
    this.draw();
  }
  ...
  getValue(key) {
    ...
    switch (key) {
      ...
      case 'TotalVolume':
        return this.getValue('VolumePerKG') * this.getValue('DoseWeightKG');
      
      case 'NonLipidVolume':
      case 'NonLipidVolTotal':
        return this.getValue('TotalVolume') - this.getValue('LipidVolTotal');
...
export function isValidKey(key) {
  ...
  return allValidKeys.includes(key);
}
