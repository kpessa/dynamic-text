...
export class ReferenceService {
  ...
  async saveReference(
    ...
    return errorService.withRetry(async () => {
      ...
      const referenceRef = doc(database, COLLECTIONS.INGREDIENTS, ingredientId, 'references', referenceId);
      ...
      const data: any = {
        ...
        validationStatus: referenceData.validationStatus || (currentData ? currentData['validationStatus'] : null) || 'untested',
        validationNotes: referenceData.validationNotes || (currentData ? currentData['validationNotes'] : null) || '',
        validatedBy: referenceData.validatedBy || (currentData ? currentData['validatedBy'] : null) || null,
        validatedAt: referenceData.validatedAt || (currentData ? currentData['validatedAt'] : null) || null,
        testResults: referenceData.testResults || (currentData ? currentData['testResults'] : null) || null,
      ...
      if (currentDoc.exists() && currentData) {
        await this.saveReferenceVersionHistory(ingredientId, referenceId, {
          ...currentData,
          commitMessage: currentData['commitMessage'] || null
        });
      }
      ...
      const ingredientData = ingredientDoc.exists() ? ingredientDoc.data() : null;
      const ingredientVersion = ingredientData ? (ingredientData['version'] || 0) : 0;
  ...
  async getReferencesForIngredient(ingredientId: string): Promise<ServiceResponse<ReferenceData[]>> {
    return errorService.withRetry(async () => {
      ...
      const references = await cacheService.get(
        ...
        async () => {
          const q = query(
            collection(database, COLLECTIONS.INGREDIENTS, ingredientId, 'references'),
      ...
      const reference = await cacheService.get(
        ...
        async () => {
          const docSnapshot = await getDoc(
            doc(database, COLLECTIONS.INGREDIENTS, ingredientId, 'references', referenceId)
          );
  ...
  async getReferencesForComparison(
    ...
    return errorService.withRetry(async () => {
      ...
      const database = db;
      const colRef = collection(database, COLLECTIONS.INGREDIENTS, ingredientId, 'references');
      
      const q = healthSystem 
        ? query(colRef, where('healthSystem', '==', healthSystem))
        : colRef;
      ...
      references.forEach(ref => {
        const popType = ref.populationType || 'unknown';
        if (!grouped[popType]) {
          grouped[popType] = [];
        }
  ...
  async getReferenceVersionHistory(
    ...
    return errorService.withRetry(async () => {
      ...
      const versions = await cacheService.get(
        ...
        async () => {
          const q = query(
            collection(database, COLLECTIONS.INGREDIENTS, ingredientId, 'references', referenceId, 'versions'),
