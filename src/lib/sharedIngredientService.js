 ...
 */
export async function addToSharedIngredient(sharedId, reference) {
  try {
    const user = getCurrentUser();
    if (!user) throw new Error('User not authenticated');
    ...
    if (masterDoc.exists()) {
      await updateDoc(masterRef, {
        sharedCount: (masterDoc.data().sharedCount || 0) + 1
      });
    }
 ...
 */
export async function getSharedIngredientByHash(hash) {
  try {
 ...
 */
export async function removeFromSharedIngredient(sharedId, reference) {
  try {
    const user = getCurrentUser();
    if (!user) throw new Error('User not authenticated');
 ...
 */
export async function getSharedIngredients() {
  try {
 ...
 */
export async function getSharedIngredient(sharedId) {
  try {
 ...
 */
export async function isIngredientShared(ingredientId, configId = null) {
  try {
    if (configId) {
 ...
 */
export async function getSharedIngredientInfo(ingredientId) {
  try {
 ...
 */
export async function findSharingCandidates(ingredientId) {
  try {
    const ingredientDoc = await getDoc(doc(db, COLLECTIONS.INGREDIENTS, ingredientId));
    if (!ingredientDoc.exists()) {
      return [];
    ...
    for (const doc of snapshot.docs) {
      if (doc.id !== ingredientId) {
        ...
        const references = refsSnapshot.docs.map(refDoc => ({
          id: refDoc.id,
          ingredientId: doc.id,
          ...refDoc.data()
        }));
 ...
 */
export async function makeIndependentCopy(ingredientId, configId) {
  try {
    const user = getCurrentUser();
    if (!user) throw new Error('User not authenticated');
