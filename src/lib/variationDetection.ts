 ...
 */
function extractTextFromSections(sections: Section[]): string {
 ...
 * @param noteArray - Array of NOTE objects
 * @returns Concatenated text content
 */
function extractTextFromNotes(noteArray: NoteItem[]): string {
...
export function calculateIngredientSimilarity(
  ingredient1: IngredientData | any, 
  ingredient2: IngredientData | any
): number {
...
export function findVariations(
  ...
  const variations: VariationMatch[] = [];
 ...
 * @param targetName - Name to find variations of
 * @param ingredients - Array of ingredients
 * @param threshold - Similarity threshold
 * @returns Ingredients with similar names
 */
export function findNameVariations(
  targetName: string, 
  ingredients: (IngredientData | any)[], 
  threshold: number = 0.8
): Array<{ ingredient: IngredientData | any; name: string; similarity: number; similarityPercent: number }> {
  if (!targetName || !ingredients) return [];
  
  const variations: Array<{ ingredient: IngredientData | any; name: string; similarity: number; similarityPercent: number }> = [];
 ...
 * @returns Array of variation groups
 */
export function clusterVariations(
  ingredients: IngredientData[], 
  threshold: number = 0.7
): VariationCluster[] {
  if (!ingredients || !Array.isArray(ingredients)) return [];
  
  const clusters: VariationCluster[] = [];
  ...
  ingredients.forEach(ingredient => {
    ...
    if (variations.length > 0) {
      const cluster: VariationCluster = {
 ...
 * @param ingredients - Array of ingredients
 * @param mergeThreshold - Threshold for merge suggestion (default 0.85)
 ...
 * @returns Object with highlighted differences
 */
export function highlightDifferences(text1: string, text2: string): TextDifferences {
  const words1 = text1.split(/\s+/);
  const words2 = text2.split(/\s+/);
  
  const result1: DifferenceHighlight[] = [];
  const result2: DifferenceHighlight[] = [];
 ...
 * @param ingredients - Array of ingredients
 * @returns Statistics about variations
 */
export function getVariationStats(ingredients: IngredientData[]): VariationStats {
