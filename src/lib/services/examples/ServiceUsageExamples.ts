 ...
 */
class IngredientManagementExample {
 ...
 */
class ReferenceManagementExample {
  async demonstrateReferenceOperations() {
    try {
      ...
      if (!ingredient) {
        console.log('❌ No ingredient found to work with');
        return;
      }
      ...
      if (saveRefResult.success) {
        ...
        if (pediatricRefs.success && pediatricRefs.data) {
      ...
      if (allRefsResult.success && allRefsResult.data) {
  ...
  async demonstrateConfigImport() {
    try {
      ...
      if (importResult.success && importResult.data) {
        const { configId, importStats } = importResult.data;
        ...
        if (configIngredientsResult.success && configIngredientsResult.data) {
...
class ErrorHandlingExample {
  ...
  async demonstrateMedicalSafetyValidation() {
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'type' in error && (error as any).type === 'MEDICAL_SAFETY_ERROR') {
        console.log('🚨 Medical safety error caught:', (error as any).userMessage);
        console.log('Technical details:', (error as any).technicalDetails);
      } else {
