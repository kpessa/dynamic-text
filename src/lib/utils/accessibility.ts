...
class ScreenReaderAnnouncer {
  ...
  announceTPNCalculation(calculationType: string, value: string | number, unit = '', isWarning = false): void {
  ...
  announceValidationResult(isValid: boolean, fieldName: string, errorMessage = ''): void {
  ...
  announceSectionChange(sectionName: string, actionType: string): void {
...
export class FocusManager {
  ...
  private trapElements: TrapElement[] = [];

  // Store current focus and move to new element
  moveFocusTo(element: HTMLElement | null, storePrevious = true): void {
  ...
  restoreFocus(): void {
    ...
    if (previousElement && 'focus' in previousElement && typeof previousElement.focus === 'function') {
...
export class KeyboardManager {
  ...
  registerShortcut(key: string, callback: (e: KeyboardEvent) => void, description: string, ctrlKey = false, altKey = false, shiftKey = false): void {
  ...
  private handleEscapeKey(): void {
    ...
    const openModals = document.querySelectorAll<HTMLElement>('.modal-backdrop');
    const openDropdowns = document.querySelectorAll<HTMLElement>('.dropdown-open');
  ...
  getShortcutsList(): Array<{ key: string; description: string }> {
...
export class ColorContrastManager {
  ...
  static meetsWCAGAA(color1: string, color2: string, largeText = false): boolean {
    const ratio = this.getContrastRatio(color1, color2);
    return largeText ? ratio >= 3 : ratio >= 4.5;
  }

  static meetsWCAGAAA(color1: string, color2: string, largeText = false): boolean {
    const ratio = this.getContrastRatio(color1, color2);
  ...
  static getHighContrastColor(backgroundColor: string, preferredColor: string | null = null): string {
...
export class AccessibleValidation {
  static createFieldError(fieldId: string, errorMessage: string, isLive = true): HTMLDivElement {
    const errorId = `${fieldId}-error`;
    let errorElement = document.getElementById(errorId) as HTMLDivElement | null;
  ...
  static validateTPN(value: string, fieldName: string, min: number | null = null, max: number | null = null, unit = ''): ValidationResult {
    const errors: string[] = [];
...
keyboardManager.registerShortcut('n', () => {
  const newButton = document.querySelector<HTMLButtonElement>('[data-action="new"]');
  if (newButton) newButton.click();
}, 'Create new document', true);

keyboardManager.registerShortcut('t', () => {
  const testButton = document.querySelector<HTMLButtonElement>('[data-action="run-tests"]');
  if (testButton) testButton.click();
}, 'Run all tests', true);

keyboardManager.registerShortcut('k', () => {
  const keyRefButton = document.querySelector<HTMLButtonElement>('[data-action="toggle-keys"]');
  if (keyRefButton) keyRefButton.click();
}, 'Toggle TPN key reference', true);

keyboardManager.registerShortcut('i', () => {
  const ingredientButton = document.querySelector<HTMLButtonElement>('[data-action="ingredients"]');
