...
/**
 * Convert clinical notes from NOTE array format to sections format
 * 
 * NOTE Format Specification:
 * - Array of objects with {TEXT: string} property
 * - Static content: regular text lines
 * - Dynamic content: wrapped between [f( and )] markers
 * - Dynamic blocks can span multiple NOTE items
 * 
 * @param notes - Array of NOTE items from clinical systems
 * @returns Array of Section objects with type and content
 * 
 * @example
 * Input: [
 ...
 *   {id: "2", type: "dynamic", content: "return me.getValue('weight') + ' kg'"}
 * ]
 */
function convertNotesToSections(notes?: NoteItem[]): Section[] {
  ...
  let sectionId = 1;
  let dynamicNestLevel = 0; // Track nesting level for error detection
  ...
  notes.forEach((note, noteIndex) => {
    ...
    // Check for single-line dynamic blocks first (e.g., "[f(return 'Hello')]")
    if (text.includes('[f(') && text.includes(')]')) {
      // Handle potentially multiple dynamic blocks in one line
      let remainingText = text;
      let hasStaticBefore = false;
      ...
      while (remainingText.includes('[f(')) {
        ...
        if (startIdx > 0) {
          ...
          if (beforeText.trim()) {
            // If we have accumulated static content, add it to the before text
            if (currentStaticContent.trim()) {
              sections.push({ 
                id: String(sectionId++),
                type: 'static', 
                content: currentStaticContent.trim() + '\n' + beforeText.trim(),
                name: `Section ${sectionId - 1}`
              });
            ...
            hasStaticBefore = true;
          }
        } else if (currentStaticContent.trim() && !hasStaticBefore) {
        ...
        if (endIdx !== -1) {
          ...
          const firstLine = dynamicText.trim().split('\n')[0];
          const sectionName = firstLine.startsWith('//') 
            ? firstLine.substring(2).trim() 
            : `Dynamic Section ${sectionId}`;
          ...
          // No closing )] on this line, this starts a multi-line block
          inDynamicBlock = true;
          dynamicContent = remainingText.substring(startIdx);
          dynamicNestLevel = (dynamicContent.match(/\[f\(/g) || []).length;
          dynamicStarted = true;
          break;
        }
      }
      ...
      if (!inDynamicBlock && remainingText.trim()) {
        currentStaticContent = remainingText;
      }
    } else if (text.includes('[f(')) {
      // Multi-line dynamic block start
      // Save any accumulated static content before starting dynamic block
      ...
      inDynamicBlock = true;
      dynamicContent = text;
      dynamicNestLevel = (text.match(/\[f\(/g) || []).length;
      ...
      dynamicNestLevel += (text.match(/\[f\(/g) || []).length;
      dynamicNestLevel -= (text.match(/\)\]/g) || []).length;
      
      // Check if dynamic block is complete
      if (text.includes(')]') && dynamicNestLevel <= 0) {
        ...
        while (remainingText.includes('[f(')) {
          ...
          if (endIdx !== -1) {
            const dynamicText = remainingText.substring(startIdx + 3, endIdx);
            // Extract first line as potential section name
            const firstLine = dynamicText.trim().split('\n')[0];
            const sectionName = firstLine.startsWith('//') 
              ? firstLine.substring(2).trim() 
              : `Dynamic Section ${sectionId}`;
            ...
            // Unclosed dynamic block - treat as static with warning
            console.warn(`Unclosed dynamic block detected at note index ${noteIndex}`);
            if (remainingText.trim()) {
              sections.push({ 
                id: String(sectionId++),
                type: 'static', 
                content: remainingText.trim(),
                name: `Section ${sectionId - 1} (Warning: Unclosed dynamic block)`
              });
            }
            break;
          }
  ...
  // Handle unclosed dynamic block at end of notes
  if (inDynamicBlock && dynamicContent.trim()) {
    console.warn(`Unclosed dynamic block at end of notes. Nesting level: ${dynamicNestLevel}`);
    // Save the unclosed dynamic content as static with a warning
    sections.push({ 
      id: String(sectionId++),
      type: 'static', 
  ...
  } else if (currentStaticContent.trim()) {
...
export class ConfigService {
  ...
  async saveImportedConfig(
    ...
    return errorService.withRetry(async () => {
      ...
      const database = db;
      const baselineRef = doc(database, 'baselineConfigs', configId);
      ...
      const configRef = doc(database, 'importedConfigs', configId);
  ...
  async getAllConfigs(): Promise<ServiceResponse<ImportedConfig[]>> {
    return errorService.withRetry(async () => {
      ...
      const database = db; // Capture db in local variable for TypeScript
      const configs = await cacheService.get(
        CACHE_KEYS.ALL_CONFIGS,
        async () => {
          const q = query(
            collection(database, 'importedConfigs'),
  ...
  async getConfig(configId: string): Promise<ServiceResponse<ImportedConfig | null>> {
    return errorService.withRetry(async () => {
      ...
      const config = await cacheService.get(
        ...
        async () => {
          const docSnapshot = await getDoc(doc(database, 'importedConfigs', configId));
  ...
  async getConfigIngredients(configId: string): Promise<ServiceResponse<ImportedIngredient[]>> {
    return errorService.withRetry(async () => {
      ...
      const ingredients = await cacheService.get(
        ...
        async () => {
          const q = query(
            collection(database, 'importedConfigs', configId, 'ingredients'),
            orderBy('index', 'asc')
          );
  ...
  async getBaselineConfig(configId: string): Promise<ServiceResponse<any>> {
    return errorService.withRetry(async () => {
      ...
      const baseline = await cacheService.get(
        ...
        async () => {
          const baselineDoc = await getDoc(doc(database, 'baselineConfigs', configId));
  ...
  async revertToBaseline(
    ...
    return errorService.withRetry(async () => {
      ...
      const ingredientId = normalizeIngredientId(ingredientName);
      const referenceRef = doc(database, COLLECTIONS.INGREDIENTS, ingredientId, 'references', configId);
  ...
  async deleteConfig(configId: string): Promise<ServiceResponse<void>> {
    return errorService.withRetry(async () => {
      ...
      batch.delete(doc(database, 'importedConfigs', configId));
      
      // Also delete baseline if it exists
      batch.delete(doc(database, 'baselineConfigs', configId));
  ...
  private async getBaselineIngredients(configId: string): Promise<ServiceResponse<any[]>> {
    return errorService.withRetry(async () => {
      ...
      const ingredients = await cacheService.get(
        ...
        async () => {
          const q = query(
            collection(database, 'baselineConfigs', configId, 'ingredients'),
            orderBy('index', 'asc')
          );
  ...
  private async processConfigIngredients(
    ...
    if (saveBaseline) {
      configData.INGREDIENT.forEach((ingredient, index) => {
        const baselineIngredientRef = doc(collection(database, 'baselineConfigs', configId, 'ingredients'));
    ...
    for (const [name, ingredientInfo] of processedIngredients) {
      const ingredientId = normalizeIngredientId(name);
      const ingredientRef = doc(database, COLLECTIONS.INGREDIENTS, ingredientId);
    ...
    configData.INGREDIENT.forEach((ingredient, index) => {
      const ingredientRef = doc(collection(database, 'importedConfigs', configId, 'ingredients'));
  ...
  /**
   * Convert clinical notes to sections format
   * Public method that wraps the internal convertNotesToSections function
   */
  convertNotesToSections(notes?: NoteItem[]): Section[] {
    return convertNotesToSections(notes);
  }
}
...
// Also export the function directly for backwards compatibility
export { convertNotesToSections };